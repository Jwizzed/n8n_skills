...
7. **[Content Factory Pipeline](content_factory_pipeline.md)**
    - Deterministic research-to-content pipeline with asset management and multi-persona routing
    - Pattern: Form Input → Deduplicate → Research → Extract Assets → Switch by Pillar → Specialized Writer → Output Ledger
...
**Content Factory Pipeline** - Use when:
- Generating content from research with deterministic routing
- Need deduplication checks before processing
- Different writing personalities/styles for the same input
- Managing downloadable assets (images, docs) alongside content
- Example: "Form inputs topic → Perplexity researches → Gemini writes script → Save to Sheets + Drive"
...
- **[content_factory_pipeline.md](content_factory_pipeline.md)** - Deterministic research-to-content pipelines with Switch-based multi-persona routing
