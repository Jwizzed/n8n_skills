...
## Additional Real-World Errors (From Live Builds)

### 10. Google Sheets — "Range is required for read operation"

**What it means**: When using `operation: "read"` with `filtersUI`, the `range` field is still required at the top level of `parameters`.
...
```javascript
{
  ...
  "filtersUI": {
    "values": [{ "lookupColumn": "Topic", "lookupValue": "={{ $json.Topic_Seed }}" }]
  }
  // Missing: range
}
```
...
**Note**: Putting `range` inside `options.dataLocationOnSheet.values.range` does NOT satisfy this requirement for filtered reads.

---

### 11. Google Drive — "Invalid operation 'folderCreate'"

**What it means**: Google Drive node does not have a `folderCreate` operation. Use `resource: "folder"` + `operation: "create"` instead.

**Error**:
```json
{
  "node": "Drive Create Folder",
  "message": "Invalid operation \"folderCreate\" for node n8n-nodes-base.googleDrive. Similar operations: \"create\", \"update\""
}
```
...
```javascript
{
  "resource": "folder",     // ✅ Set resource first
  "operation": "create",    // ✅ Then operation
  "name": "My Folder"
}
```
...
### 12. Expression Format — "Mixed literal text and expression requires = prefix"

**What it means**: A string field contains both literal text and n8n `{{...}}` expressions, but does not start with `=`.

**Error**:
```json
{
  "node": "Gemini Curiosity",
  "message": "Expression format error: Mixed literal text and expression requires = prefix for expression evaluation"
}
```
...
**Rule of thumb**:
- Pure expression: `"={{ $json.name }}"` ✅
- Pure literal: `"Hello world"` ✅
- Mixed literal + expression: `"=Hello {{ $json.name }}"` ✅ (must start with =)

---

### 13. IF Node — Invalid Operator Type

**What it means**: IF node v2 `operator.type` must be a data type (`string`, `number`, `boolean`, `dateTime`, `array`, `object`), not an operation name like `exists`.

**Error**:
```json
{
  "node": "If Duplicate",
  "message": "conditions.conditions[0].operator: invalid type \"exists\". Type must be a data type..."
}
```
...
{
  "operator": {
    "type": "exists",        // ❌ "exists" is an operation, not a type
    "operation": "exists"
  }
}
```
...
```javascript
{
  "operator": {
    ...
    "operation": "notEquals" // ✅ Operation name
  }
}
```

**Valid data types for IF v2**: `string`, `number`, `boolean`, `dateTime`, `array`, `object`
...
### 14. IF Node — Unary Operator Requires `singleValue: true`

**What it means**: Unary operators (`isEmpty`, `isNotEmpty`) only need `value1`, but the validator requires `singleValue: true` to be explicit. However, **auto-sanitization fixes this on save** — you usually don't need to handle it manually.

**Error**:
```json
{
  "node": "If Duplicate",
  "message": "unary operator \"isEmpty\" requires \"singleValue: true\""
}
```

**What to do**: Let auto-sanitization handle it, or add `"singleValue": true` if pre-validating before save.
