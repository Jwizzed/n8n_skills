...
#### Read with Filters (Deduplication Pattern)

When reading a sheet to check if a value exists (deduplication/lookup), use `operation: "read"` with `filtersUI` AND a top-level `range`:
...
```javascript
{
  ...
  "range": "A:Z",                      // ← REQUIRED even with filters
  "filtersUI": {
    "values": [
      {
        "lookupColumn": "Topic",
  ...
  "combineFilters": "AND",
  "returnFirstMatch": true             // ← Stops after first match
}
```

**Key insight**: `range` must be at the top level of `parameters`, NOT nested inside `options.dataLocationOnSheet`. The validator throws `"Range is required for read operation"` if omitted.

#### Append Row (Production Queue Pattern)
...
```javascript
{
  ...
  "columns": {
    ...
    "value": null
    // After selecting the sheet in the UI, map columns here
  }
}
```
...
There is **no** `operation: "folderCreate"`. Use the standard resource/operation pattern:
...
### HTTP Request Node — API Integration Patterns

#### POST JSON to External API (Perplexity, Gemini, OpenAI)
...
**Key rules**:
- Use `"contentType": "json"` and put the JSON directly in `body`.
- Expressions inside `body` strings must start with `=` when mixed with literal text.
- For Gemini/Google APIs, use `"genericAuthType": "googleApi"` or pass key via `queryParameters`.

#### Download Binary File (Image/Media)
...
Setting `neverError: true` allows the workflow to continue even if a single media URL is broken (common when processing batches of scraped URLs).

---

### Switch Node v2 — Multi-Route Routing
...
**Connections wiring**: Each rule's `output` index maps to `main[index]` in the `connections` object. Ensure you have the same number of `main` arrays as rules.
